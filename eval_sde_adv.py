...
class SDE_Adv_Model(nn.Module):
    def __init__(self, args, config):
        super().__init__()
        self.args = args
        self.num_samples = getattr(args, "num_samples", 1)
    ...
    def classify_with_sampling(self, x):
        ...
        for i in range(self.num_samples):
            ...
            x_re = self.runner.image_editing_sample(
                (x_resized - 0.5) * 2, bs_id=self.counter.item() + i, tag=self.tag
            )
    ...
    def forward(self, x):
        ...
        if self.num_samples > 1:
            out = self.classify_with_sampling(x)
        else:
            if "imagenet" in self.args.domain:
                x = F.interpolate(
                    x, size=(256, 256), mode="bilinear", align_corners=False
...
def eval_autoattack(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    print(
        f"attack_version: {attack_version}, attack_list: {attack_list}"
    ...
    adversary_resnet = AutoAttack(
        classifier,
        norm=args.lp_norm,
        eps=args.adv_eps,
        version=attack_version,
        attacks_to_run=attack_list,
        log_path=f"{log_dir}/log_resnet.txt",
    ...
    if attack_version == "rand":
        ...
        print(
            f"[classifier] rand version with eot_iter: {adversary_resnet.apgd.eot_iter}"
        )
    print(f"{args.lp_norm}, epsilon: {args.adv_eps}")

    x_adv_resnet = adversary_resnet.run_standard_evaluation(
    ...
    adversary_sde = AutoAttack(
        model,
        norm=args.lp_norm,
        eps=args.adv_eps,
        version=attack_version,
        attacks_to_run=attack_list,
        log_path=f"{log_dir}/log_sde_adv.txt",
        device=config.device,
    )
    if attack_version == "custom":
...
def eval_stadv(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    print(
        "initial accuracy: {:.2%}, time elapsed: {:.2f}s".format(
            init_acc, time.time() - start_time
        )
    )

    adversary_resnet = StAdvAttack(
        resnet, bound=args.adv_eps, num_iterations=100, eot_iter=args.eot_iter
    )
    ...
    print(
        "robust accuracy: {:.2%}, time elapsed: {:.2f}s".format(
            robust_acc, time.time() - start_time
        )
    )
...
def robustness_eval(args, config):
    middle_name = (
        "_".join([args.diffusion_type, args.attack_version])
    ...
    log_dir = os.path.join(
        ...
        middle_name,
        "seed" + str(args.seed),
        "data" + str(args.data_seed),
    )
    os.makedirs(log_dir, exist_ok=True)
    args.log_dir = log_dir
    logger = utils.Logger(
        file_name=f"{log_dir}/log.txt", file_mode="w+", should_flush=True
    )
...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--diffusion_type", type=str, default="ddpm", help="[ddpm, sde]"
    )
    parser.add_argument(
        "--score_type",
        type=str,
        default="guided_diffusion",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of sampling runs for averaging (default: 1)",
    )
    ...
    parser.add_argument(
        "--step_size", type=float, default=1e-3, help="step size for ODE Euler method"
    )
