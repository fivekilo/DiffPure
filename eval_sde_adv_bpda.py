...
class SDE_Adv_Model(nn.Module):
    ...
    def purify(self, x):
        ...
        if counter % 5 == 0:
            ...
            print(
                "Sampling time per batch: {:0>2}:{:05.2f}".format(int(minutes), seconds)
            )
    ...
    def forward(self, x, mode="purify_and_classify"):
        if self.num_samples > 1 and mode == "purify_and_classify":
...
def eval_bpda(args, config, model, x_val, y_val, adv_batch_size, log_dir):
    ...
    adversary_resnet = BPDA_EOT_Attack(
        resnet_bpda,
    ...
    start_time = time.time()
    class_batch, ims_adv_batch = adversary_resnet.attack_all(
        x_val, y_val, batch_size=adv_batch_size
    )
    init_acc = float(class_batch[0, :].sum()) / class_batch.shape[1]
    robust_acc = float(class_batch[-1, :].sum()) / class_batch.shape[1]

    print(
        "init acc: {:.2%}, robust acc: {:.2%}, time elapsed: {:.2f}s".format(
            init_acc, robust_acc, time.time() - start_time
        )
    )
    ...
    model_.reset_counter()
    model_.set_tag()
    class_batch, ims_adv_batch = adversary_sde.attack_all(
        x_val, y_val, batch_size=adv_batch_size
    )
    init_acc = float(class_batch[0, :].sum()) / class_batch.shape[1]
    robust_acc = float(class_batch[-1, :].sum()) / class_batch.shape[1]
...
def parse_args_and_config():
    ...
    parser.add_argument(
        "--diffusion_type", type=str, default="ddpm", help="[ddpm, sde, celebahq-ddpm]"
    )
    parser.add_argument(
        "--score_type",
        type=str,
