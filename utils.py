...
def get_accuracy(model, x_orig, y_orig, bs=64, device=torch.device("cuda:0")):
    ...
    for counter in range(n_batches):
        x = (
            x_orig[counter * bs : min((counter + 1) * bs, x_orig.shape[0])]
            .clone()
            .to(device)
        )
        y = (
            y_orig[counter * bs : min((counter + 1) * bs, x_orig.shape[0])]
            .clone()
            .to(device)
        )
        output = model(x)
...
def get_accuracy_with_sampling(
    model, x_orig, y_orig, bs=64, num_samples=1, device=torch.device("cuda:0")
):
    """
    使用多次采样平均的方式计算模型精度
    ...
    for counter in range(n_batches):
        ...
        acc += (avg_output.max(1)[1] == y).float().sum()
...
def get_image_classifier(classifier_name):
    ...
    if "imagenet" in classifier_name:
        ...
        if "wideresnet-28-10" in classifier_name:
            ...
            model.load_state_dict(
                update_state_dict(torch.load(model_path), idx_start=7)
            )
            model.eval()
...
def load_data(args, adv_batch_size):
    if "imagenet" in args.domain:
        ...
        val_data = data.imagenet_lmdb_dataset_sub(
            val_dir,
